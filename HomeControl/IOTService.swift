...
class IOTService {
    ...
    let ipAddress = "http://127.0.0.1"
    
    func fetchTemperature(fetchedHomeTemperatureCallBack:(Int, NSError?, HomeModel?) -> ()){
        
        let temperatureURL = NSURL(string: "\(ipAddress):3000/temperature")
    ...
    func switchLamp(state:Bool,switchLampCallBack:(Int, NSError?) -> ()) {
        ...
        let switchLampURL = NSURL(string: "\(ipAddress):3000/switchlamp")
        let session = NSURLSession.sharedSession()
        let request = NSMutableURLRequest(URL: switchLampURL!)
    ...
    func fetchLampState(switchLampCallBack:(Int, NSError?) -> ()) {
        ...
        let fetchLampStateURL = NSURL(string: "\(ipAddress):3000/switchlamp")
        let session = NSURLSession.sharedSession()
        let request = NSMutableURLRequest(URL: fetchLampStateURL!)
